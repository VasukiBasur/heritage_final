...
<script>
    ...
    function initMap() {
        ...
        deliveries.forEach((delivery, index) => {
            ...
            // HTML Markers
            // Start - Static small dot
            new HTMLMarker(delivery.start, `<div class="w-2 h-2 rounded-full border border-black" style="background-color: ${delivery.color}; box-shadow: 0 0 10px ${delivery.color};"></div>`, mapInstance, {x:0,y:0}, 1);
            
            // End - Pulsing Big Dot
            new HTMLMarker(delivery.end, `<div class="w-5 h-5 rounded-full border-2 shadow-[0_0_15px_5px_currentColor] animate-pulse" style="color: ${delivery.color}; background-color: #1a1a1a; border-color: ${delivery.color};"></div>`, mapInstance, {x:0,y:0}, 2);

            // Floating Route Badge (Mid)
            const badgeHtml = `
                <div class="px-2 py-1 rounded-md shadow-[0_4px_15px_rgba(0,0,0,0.5)] border backdrop-blur text-[9px] font-bold uppercase tracking-widest pointer-events-none" 
                     style="background: rgba(10,10,10,0.85); border-color: ${delivery.color}80; color: ${delivery.color}; min-width: 100px; text-align: center;">
                    ${delivery.status} <br/>
                    <span class="text-white text-[8px]">${delivery.eta} | ${delivery.dist}</span>
                </div>
            `;
...
print('supplier_tracking.html completely rebuilt with 8 routes.')
