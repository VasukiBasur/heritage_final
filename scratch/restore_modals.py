...
if "id=\"profileModal\"" not in content:
    # dashboard.html ends with {% endblock %}
...
# 3. Fix the search list onclick handler
# Replace the old broken escaped onclick with the proper modal trigger
broken_onclick = r"onclick=\\\"document.getElementById\('searchResults'\).classList.add\('hidden'\); window.addMockLog\('\$\{item.name\}'\); document.querySelector\('table'\).scrollIntoView\(\{behavior: 'smooth'\}\);\\\""
fixed_onclick = "onclick=\"openProfileModal('${item.name}', '${item.type}', '${item.desc}', '${item.icon}')\""
content = re.sub(broken_onclick, fixed_onclick, content)

with open('d:\\dbmss\\templates\\dashboard.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("Modals and JS logic restored to dashboard.html")
